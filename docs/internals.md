...
| `GET /healthz` | Liveness for the keep-alive pinger and Render's health check. Mounted before the cookie and visit middleware, so a ping sets no cookie and writes no event. |
...
Free-tier realities: the instance spins down after ~15 min idle, so the first request after a quiet spell waits ~30–60 s on the platform cold start (the app itself boots in ~1 s — baked model, committed vectors). Measured in production the app boot itself averages ~14 s on the free tier's shared CPU (the `boot` event on `/stats.html`), and one week showed 213 boots for 292 visitors — most visitors were waiting on a cold start. A free external cron hitting `https://onebysec.com/healthz` every 10 minutes keeps the instance warm; it must target `/healthz`, not `/`, because `/` records a visit. Memory is fine: ~101 MiB RSS against the 512 MB cap; `DENSE_DISABLED=1` is the escape hatch if that ever changes.
